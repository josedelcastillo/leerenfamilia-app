...
/* Fallback where :has() is unsupported: focus must never be invisible. */
.opcion:focus-within { outline: 3px solid var(--accent); outline-offset: 2px; }
...
.confirmado__check {
  ...
  display: grid; place-items: center; background: var(--ok); color: #ffffff; font-size: 19px;
}
.banda-estado {
...
.banner--offline { color: var(--warn); background: var(--warn-soft); border-color: var(--warn-line); }
.banner--pending { color: var(--accent); }
.btn--secondary { min-height: var(--touch); background: var(--paper); color: var(--ink); border: 1.5px solid var(--line-strong); font-size: 17px; font-weight: 400; }
