...
1. **Ancla del cronograma.** `01` §4 acompaña "las primeras 8 semanas de **vida**"; el prompt calcula la semana desde la **fecha de registro**. Si el 1.er control ocurre a las 2–3 semanas de vida, la semana 8 del programa cae fuera de las 8 semanas de vida. El propio prompt se contradice: la tabla de entidades dice que la fecha de nacimiento "define el cronograma", la fórmula usa `fecha_registro`. **Bloqueante para Fase 2.** → Resuelto en `docs/decisiones.md` D-003: ancla a la fecha de ingreso al programa, paramétrico.
